...
## Part 3: Introduction to demographic modelling - March 18, 2026 
...
Before to start, copy the content of the repository (copy the whole command below, i.e. from the "cp" to the "~" including the spaces!):
...
Please note that commands including the program smc++ are just provided (i.e. smc++ vcf2smc, smc++ estimate & smc++ plot below), please read the commands to understand the rationale but DO NOT execute them on the cluster.
...
Try to know how many biallelic SNPs are present in the VCF file? Is it surprising?

For all the work below, I performed simulations with this subset, so first I extract the names of the 3 first scaffolds in the vcf (I focused on the first three to speed up the computations, but in general we work on all scaffolds (at least those > 100kb)). Special note for people working on RADseq, this method is not adapted for your data, here we should have WGS data! So if you execute the command below you will see a difference when you will read the file, because it is a subset. 
cd ~/Day3_demography/data-datepalm/
...
VERY IMPORTANT: The commands provided below have the objective to give you an idea of the expected scripts. 
...
AGAIN DO NOT EXECUTE BELOW THE COMMAND, but try to understand them and especially what is perform at each step. Restart to use the commands at the SKYLINE PLOT VISUALISATION stage.
...
Then we could use another smc++ command (smc ++ estimate), a key step for demographic inference. This command estimates effective population size changes over time using coalescent patterns inferred from the polymorphism data.
cd ~/Day3_demography/data-datepalm/Calling/
...
Finally, we could use a smc++ function to generate the final output files! To do so, the following command would be needed:
...
SKYLINE PLOT VISUALISATION: The command smc++ plot generates a demographic history plot from SMC++ results, providing a visual representation of effective population size (N~e~​) over time. We can refine a bit the output using R.
...
Please make sure that you are no longer in a specific conda environment but in conda base. In case, you are still in the Workshop_TL_YB_Calling environment, leave this conda environment (conda deactivate). Note that some warnings will be shown on the screen (ending with a + null device, 1)! Note that the R script is available on the cluster but, if you prefer to use Rstudio, you can download it as well as the physalia_smcpp_plot_missingcutoff500.csv file to run it on your own computer.
...
OPTIONAL (level 1): Modify the R script to explore the results of the same smc++ analysis but assuming a different --missing-cutoff option (namely 50000). The file "physalia_smcpp_plot_missingcutoff50kb.csv" is availeble on Res_SMCpp directory and was generated from smc++ estimate (models also made available on the following directory: ~/Day3_demography/data-datepalm/Res_SMCpp/res_SMCpp_missing50kb/). Compare the two results. Are there significant differences? If so, identify the population(s) where these differences occur.
...
Note: GONE provides results for the last ~600 generations, , but these may correspond to very small genomic regions due to recombination over generations, especially in species such as honey bees, which have particular recombination characteristics (as you may have noticed earlier ;)). Therefore, I strongly recommend avoiding plots that extend beyond the last 200 generations for most species and potentially even fewer for others (e.g. honey bees). Most importantly, refrain from overinterpreting changes in the more distant past, as these estimates become increasingly uncertain with the number of generations (i.e. which can be considered as the opposite behavior of coalescent-based approaches).
