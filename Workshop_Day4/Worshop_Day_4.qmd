...
###The expected results are already stored on the cluster. You can copy them like this:
cp -r /home/ubuntu/Share/Day4_selection/Results_expected .
```
...
Now in R, we can use the rehh package. For this session, the package is installed in the R version found in the GAPIT environment (see also the last part of the workshop on GWAS). You first need to activate this environment (in the shell) like this:

```{bash}
#| eval: false
conda activate GAPIT_env
```
...
**Think of deactivating the conda environment to start fresh for other scripts.**
