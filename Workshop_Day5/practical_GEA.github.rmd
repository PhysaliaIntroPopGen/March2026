...
## Part 5: Genotype-environment associations - March 20, 2026 
### General context
...
If you use the physalia instance, just use cp as usual (yes, today as well the whole line is needed, including the spaces and the ~ !!!):
...
Check the Poolstat version you use in your R environment. If it reports another version that the v3 (i.e. '3.1.0'), please let us know on slack. 
...
Then generate distribution for the three pairwise comparisons below (or any pairs of populations you want): 
...
dev.off()
Note: a few messages can appear ;)
...
A few messages can appear regarding the conversion of the matrix in a data.table. If the pooldata2genobaypass command above is not working properly today, please continue. The output files are not needed today.
...
OPTIONAL: try to generate a single plot containing the results of both the omega matrix and the F~ST~ matrix to check whether the results are consistent. You can use the par(mfrow=c(2,1)) function to generate the a multipanel figure (2 plots on the same figure).
...
corrplot(correlation.matrix,method="shade"); dev.off()
dev.off()
...
dev.off()
(again wait a minute or two, PLEASE ;) )
